...
static HDAudioRecorderUtil *audioRecorderUtil = nil;

@interface HDAudioRecorderUtil () <AVAudioRecorderDelegate> {
    NSDate *_startDate;
    NSDate *_endDate;
...
@implementation HDAudioRecorderUtil

#pragma mark - Public

+(BOOL)isRecording{
    return [[HDAudioRecorderUtil sharedInstance] isRecording];
}

// Start recording
+ (void)asyncStartRecordingWithPreparePath:(NSString *)aFilePath
                                completion:(void(^)(NSError *error))completion{
    [[HDAudioRecorderUtil sharedInstance] asyncStartRecordingWithPreparePath:aFilePath
                                                                  completion:completion];
}

// Stop recording
+(void)asyncStopRecordingWithCompletion:(void(^)(NSString *recordPath))completion{
    [[HDAudioRecorderUtil sharedInstance] asyncStopRecordingWithCompletion:completion];
}

// Cancel recording
+(void)cancelCurrentRecording{
    [[HDAudioRecorderUtil sharedInstance] cancelCurrentRecording];
}

+(AVAudioRecorder *)recorder{
    return [HDAudioRecorderUtil sharedInstance].recorder;
}
...
#pragma mark - Private
+(HDAudioRecorderUtil *)sharedInstance{
                                ...
                                completion:(void(^)(NSError *error))completion
{
    ...
    if(!_recorder || error)
    {
        _recorder = nil;
        if (completion) {
            error = [NSError errorWithDomain:LocalStringBundle(@"error.initRecorderFail", @"Failed to initialize AVAudioRecorder")
                                        code:-1
                                    userInfo:nil];
            completion(error);
        }
